...
func (m *Mongo) GetTrip(c context.Context, id id.TripID, accountID id.AccountID) (*TripRecord, error) {
	...
	objID, err := objid.FromID(id) // 别人给的id不确定，不能用mustFromID
	if err != nil {
...
func (m *Mongo) GetTipps(c context.Context, accountID id.AccountID, status rentalpb.TripStatus) ([]*TripRecord, error) {
	filter := bson.M{
...
func (m *Mongo) UpdateTrip(c context.Context, tid id.TripID, aid id.AccountID, updatedAt int64, trip *rentalpb.Trip) error {
	// 使用updatedAt来实现乐观锁，传入更新前的时间，更新的时候对比更新时间是否一致再更新，否则重新执行更新操作
	...
	// MatchedCount匹配数量为0不会报错，手动抛出错误
	if res.MatchedCount == 0 {
		fmt.Println(res.MatchedCount, mongo.ErrNoDocuments)
